...
export const RoleSelector = ({ selectedRole, onSelect }) => {
    ...
    return (<div className="space-y-2">
            ...
            <div className="flex gap-3">
                {roles.map((role) => (<button key={role.id} type="button" onClick={() => onSelect(role.id)} className={`flex-1 flex justify-center items-center gap-2 py-3.5 rounded-xl text-sm font-bold border transition-all duration-200 ${selectedRole === role.id

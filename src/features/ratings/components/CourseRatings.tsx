...
const CourseRatings: React.FC<CourseRatingsProps> = ({ courseId, ratings, onRatingAdded }) => {
    const { user } = useAuth();
    ...
    return (
        <Card>
            ...
            {!hasRated && user && (
                <div className="mb-6 p-4 rounded-xl bg-white/3 space-y-3">
                    ...
                    <textarea
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary/50 resize-none"
                        rows={3}
                    />
            ...
            <div className="space-y-4">
                {ratings.length === 0 ? (
                    <p className="text-sm text-text-muted">Aucun avis pour le moment.</p>
                ) : (
                    ratings.map((r) => (
                        <div key={r.id} className="flex gap-3 p-3 rounded-xl bg-white/3">
